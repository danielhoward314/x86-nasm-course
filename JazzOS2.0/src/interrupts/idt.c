...
void initIdt() {
    ...
    // 0x20 commands and 0x21 data for main PIC
    // 0xA0 commands and 0xA1 data for secondary PIC
    outPortB(0x20, 0x11);
    outPortB(0xA0, 0x11);
    ...
    // 0x8E is better understood as 0x80 and 0x0E
        // 0x80 is 8 * 16 = 128, 128 = 0b1000-0000
        // sets the DPL
        // 0x0E is E is 14, so 8 + 4 + 2 = 14, so 0b0000-1110 
        // 0b1110 is on the cited table for the interrupt gate type
...
void setIdtGate(uint8_t index, uint32_t base, uint16_t sel, uint8_t flags) {
...
void irq_handler(struct InterruptRegisters* regs) {
    ...
    // IRQs start at index 32 in the IDT:
        // setIdtGate(32, (uint32_t)irq0, 0x08, 0x8E);
    // But irq_routines starts at 0
    // so subtract 32 from the IDT interrupt number
    // to get the index into the irq_routines array
